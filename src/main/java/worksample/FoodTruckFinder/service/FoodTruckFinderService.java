...
public class FoodTruckFinderService {
	...
	/**
	 * Returns a ConditionalExpression to be used in a SoqlQuery
	 * 
	 * @param expression   a string in the format: "command=%s" where '%s' is the value to be plugged into the command
	 * @param value        
	...
	public List<FoodTruckDTO> getPage(Integer offset, Integer pageSize, Calendar cal) throws LongRunningQueryException{
		...
		try {
			...
			Expression combinedWhereClause = new CompositeExpression(expList); //Combines all generated expressions using 'AND' 
			...
			foodTrucks = consumer.query(API_RESOURCE_ID,        //Calls out to API and parses response into a list of FoodTruckDTO objects
										foodTrucksOpenNowQuery, // JSON mapping done via Jackson library
